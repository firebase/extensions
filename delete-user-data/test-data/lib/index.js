...
const questions = [
    ...
    {
        default: "./key.json",
        message: "What is the path to the service account private key for your Firebase Project?",
        name: "privateKeyPath",
        type: "input",
        validate: (value) => validateInput(value, "path to your service account private key"),
    },
];
const createFirestoreDocument = async (path, value = {
    field: "value",
}) => {
    await firebase
        .firestore()
...
const createRTDBDocument = async (path, value = { field: "value " }) => {
    await firebase
        .database()
        .ref(path)
        .set(value);
...
const createUser = async (uid, displayName) => {
    await firebase.auth().createUser({ displayName, uid });
    console.log(`Created '${displayName}' with uid: ${uid}`);
};
const uploadFile = async (projectId, filePath, destinationPath) => {
...
const populateUserDataDeletionData = async (projectId) => {
    // delete-user-data
    console.log("---------------------------------------------------------");
