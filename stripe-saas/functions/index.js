...
function subscriptionSummaryUpdate(subscription) {
  return billingUpdate({subscription: {
    id: subscription.id,
...
function verifyWebhook(next) {
  return function(req, res) {
    try {
      stripe.webhooks.constructEvent(req.rawBody.toString(), req.headers['stripe-signature'], STRIPE_SIGNING_SECRET);
      next(req, res);
    } catch (err) {
...
async function handleCustomerEvent(event) {
  ...
  if (!customer || !customer.id) {
    console.error(new Error(`[stripe-saas] unexpected payload format, no id found in ${JSON.stringify(event.data.object)}`));
    return;
  }
  if (!customer.metadata || !customer.metadata.uid) {
    console.error(new Error(`[stripe-saas] customer ${customer.id} has no uid metadata field, cannot sync`));
    return;
  }
  const uid = customer.metadata.uid;
  await usersRef.doc(uid).set(billingUpdate({customer_id: customer.id}), {merge: true});
...
async function handleSubscriptionEvent(event) {
  ...
  if (!customer.metadata || !customer.metadata.uid) {
    console.error('[stripe-saas] Customer', customer.id, 'has no uid in metadata, unable to sync subscription information.');
    return;
  }
  const uid = customer.metadata.uid;
  ...
  await db.runTransaction(async txn => {
    ...
    if (evSnap.exists) {
      console.log('[stripe-saas] skipping event', event.id, 'was already handled at', evSnap.data().handled.toDate());
      return;
    }

    txn.set(usersRef.doc(uid), subscriptionSummaryUpdate(subscription), {merge: true});
...
async function findOrCreateCustomer(auth, token) {
  ...
  if (billing && billing.customer_id) {
    if (token) {
      await stripe.customers.createSource(billing.customer_id, {source: token});
    }
  ...
  const customer = await stripe.customers.create({description,email,metadata:{uid:auth.uid}});
...
exports.subscribe = functions.https.onCall(async (data, context) => {
  if (!context.auth || !context.auth.uid) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be signed in to subscribe.');
  }
  if (!data.plan) {
    throw new functions.https.HttpsError('invalid-argument', 'Must provide a plan for subscription.');
  }
...
exports.unsubscribe = functions.https.onCall(async (data, context) => {
  if (!context.auth || !context.auth.uid) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be signed in to unsubscribe.');
  }

  const subSnap = await usersRef.doc(context.auth.uid).get();
  if (!subSnap.exists) {
    console.log('[stripe-saas] unsubscribe: no subscription found for user', context.auth.uid);
    throw new functions.https.HttpsError('failed-precondition', 'No subscription information found.');
  }
  const billing = BILLING_FIELD === '.' ? subSnap.data() : subSnap.get(BILLING_FIELD);
  if (!billing || !billing.subscription || !billing.subscription.id) {
    console.log('[stripe-saas] unsubscribe: unable to locate subscription id for user', context.auth.uid);
    throw new functions.https.HttpsError('failed-precondition', 'No subscription information found.');
  }

  return await stripe.subscriptions.update(billing.subscription.id, {cancel_at_period_end: true});
});
