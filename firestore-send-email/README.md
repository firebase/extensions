...
* SMTP connection URI: A URI representing an SMTP server this extension can use to deliver email. Note that port 25 is blocked by Google Cloud Platform, so we recommend using port 587 for SMTP connections. If you're using the SMTPS protocol, we recommend using port 465. Recommended hidden password format: `smtps://username@gmail.com@smtp.gmail.com:465`. Provide password (can use special signs) in separate field later.  Backward compatible with less secure format: `smtps://username@gmail.com:password@smtp.gmail.com:465`.
