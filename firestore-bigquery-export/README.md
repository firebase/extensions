...
* BigQuery SQL table Time Partitioning option type: This parameter will allow you to partition the BigQuery table created by the extension. You may select the granularity of partitioning based upon one of: HOUR, DAY, MONTH, YEAR. This will generate one partition per  day, hour, month or year, respectively. If no partitioning column is specified, the table will be partitioned by ingestion time (_PARTITIONTIME).

* BigQuery Time Partitioning column name: The name of the column to use for time partitioning. You can use one  of the existing columns (such as `timestamp`), or create a new column  that will be populated from a Firestore field. For built-in columns  like `timestamp`, `document_name`, `event_id`, or `operation`, you  do not need to specify a Firestore field mapping. If unspecified and  partitioning is enabled, the table will use ingestion-time partitioning  (_PARTITIONTIME). Cannot be changed if table is already partitioned.

* BigQuery SQL Time Partitioning table schema field(column) type: The BigQuery data type for the partitioning column. This parameter is required when you want to specify a custom data type for your partitioning
 field. If using a built-in field like `timestamp` without specifying this
 parameter, the field's default type will be used. If you specify this
parameter, you must also provide the Firestore field mapping, even for built-in fields. Cannot be changed if table is already partitioned.

* Firestore Document field name for BigQuery SQL Time Partitioning field option: The Firestore document field to use for populating the partitioning column. This parameter is required for custom partitioning fields or  when you specify a custom field type. For built-in fields like  `timestamp`, this is not required unless you also specify the field  type. The Firestore field value must be a TIMESTAMP, DATETIME, or DATE  compatible value. Cannot be changed if table is already partitioned. Example: Use `postDate` as the Firestore field for partitioning.
