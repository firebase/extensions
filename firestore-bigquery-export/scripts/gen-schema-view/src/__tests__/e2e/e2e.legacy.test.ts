...
describe.skip("e2e", () => {
  ...
  test("should generate a schema view based on a basic dataset and schema", async () => {
    ...
    const verifySetup = await verifySetupBQ(dataset.id, tableId, testData);

    if (!verifySetup.success) {
      console.error("Setup verification failed:", verifySetup.errors);
      // You could fail the test here if needed
