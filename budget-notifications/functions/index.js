...
_isTopicEnabled(TOPIC_NAME)
  .then(res => {
    if (!res) {
      console.log(`Creating new topic named ${TOPIC_NAME}. NOTICE: Go to Billing to connect your budget to to topic ${TOPIC_NAME}.`);
    }
  })
  .catch(err => {
    console.error(err);
...
exports.sendBudgetNotifications = async (data, context) => {
  ...
  if (await _isBillingEnabled(PROJECT_NAME)) {
    if (await _isTopicEnabled(TOPIC_NAME)) {
      ...
      console.log(`Topic was deleted. Creating new topic named ${TOPIC_NAME}. NOTICE: Go to Billing to connect your budget to to topic ${TOPIC_NAME}.`);
    }
  } else {
    console.error(`Billing is disabled for ${PROJECT_ID}. Please connect a billing account with this project.`);
  }
};
...
const _postZapier = data => {
  ...
  if (pubsubAttrs && pubsubAttrs.budgetId){
    if (pubsubData.costAmount <= pubsubData.budgetAmount) {
      console.log(`No action necessary. (Current cost: ${pubsubData.costAmount} < Budget amount: ${pubsubData.budgetAmount})`);
      return;
    }
    console.log(`(Current cost: ${pubsubData.costAmount} > Budget amount: ${pubsubData.budgetAmount}). Sending to zapier...`)
    const options = {
      method: 'POST',
    ...
    console.error(`Pubsub Message is not a Budget notification. Please verify ONLY your budget is subscribed to this topic: ${TOPIC_NAME}`);
  }
