...
## Cleanup
The `last_updated` field in Firestore is maintained to ensure the most recent transaction from the client for a given `sessionID` is reflected in Firestore. Thus, they are persisted even after the session is no longer connected.

In most use-cases, these timestamps (or tombstones) will leave a negligible footprint. If you wish to clean them up, the extension provides a cleanup function that is triggered by publishing to a PubSub topic. The function will cleanup all tombstones that have been offline for more than one week. The function/topic can be paired with Cloud Scheduler for periodic cleanups.
