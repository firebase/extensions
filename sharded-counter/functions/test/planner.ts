...
@suite class PlannerTest extends Planner {
    @test 'can handle empty shards'() {
        const plans = PlannerTest.planAggregations('/exp/cnt/_counter_shards_/\t\t\t\t0', []);
    ...
    @test 'aggregates shards into partials'() {
        let plans = PlannerTest.planAggregations('/exp/cnt/__counter_shards__/22222222', [
        ...
        plans = PlannerTest.planAggregations('/exp/cnt/__counter_shards__/00000000', [
            snap('/exp/cnt/__counter_shards__/00ec08a7'),
        ...
        plans = PlannerTest.planAggregations('/exp/cnt/__counter_shards__/01234567',
        [
    ...
    @test 'aggregates shards into counter'() {
        ...
        const plans = PlannerTest.planAggregations('/exp/cnt/__counter_shards__/\t\t\t\t0', [
            snap('/exp/cnt/__counter_shards__/00ec08a7'),
            snap('/exp/cnt/__counter_shards__/01234567')
        ]);
        expect(plans).deep.equals([{
    ...
    @test 'does not aggregate partials into themselves'() {
        const plans = PlannerTest.planAggregations('/exp/cnt/__counter_shards__/\t\t012', [
    ...
    @test 'can aggregate many counters'() {
        const plans = PlannerTest.planAggregations('/exp/cnt1/__counter_shards__/\0', [
    ...
    @test 'can aggregate partials and shards'() {
        const plans = PlannerTest.planAggregations('/exp/cnt/__counter_shards__/\t\t000', [
