...
exports.addUserToList = functions.auth.user().onCreate((user) => {
  ...
  return mailchimp.post(`/lists/${list}/members`, {
    email_address: email,
    status: 'subscribed'
  })
    .then((results) => {
      console.log(`Successfully subscribed ${email} to ${list} list. Mailchimp ID: ${results.id}`);
    }).catch((err) => {
...
exports.removeUserFromList = functions.auth.user().onDelete((user) => {
  ...
  return mailchimp.delete(`/lists/${list}/members/${hashed}`)
    .then((results) => {
