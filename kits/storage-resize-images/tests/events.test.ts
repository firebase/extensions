...
describe("setupEventChannel", () => {
  test("opens the configured channel with the selected event types", async () => {
    ...
    expect(channel).toHaveBeenCalledWith(CHANNEL, {
      allowedEventTypes: "firebase.extensions.storage-resize-images.v1.onStart",
    });
  ...
  test("stays disabled when no channel is configured", async () => {
    ...
    await events.recordStartEvent({ name: "img.png" });
...
describe("publish failures", () => {
  ...
  test("a rejected publish is logged and never reaches the caller", async () => {
    ...
    await expect(
      events.recordStartEvent({ name: "img.png" })
    ).resolves.toBeUndefined();
    await expect(
      events.recordErrorEvent(new Error("boom"))
    ).resolves.toBeUndefined();
    await expect(
      events.recordSuccessEvent({ subject: "img.png", data: {} })
    ).resolves.toBeUndefined();
    await expect(events.recordCompletionEvent({})).resolves.toBeUndefined();
    await expect(
      events.recordStartResizeEvent({ subject: "img.png", data: {} })
    ).resolves.toBeUndefined();
