...
describe("setupEventChannel", () => {
  test("opens the configured channel with the selected event types", async () => {
    ...
    expect(channel).toHaveBeenCalledWith(CHANNEL, {
      allowedEventTypes: "firebase.extensions.firestore-send-email.v1.onStart",
    });
...
describe("publish failures", () => {
  ...
  test("a rejected publish is logged and never reaches the caller", async () => {
    ...
    await expect(events.recordStartEvent(CHANGE)).resolves.toBeUndefined();
    await expect(events.recordProcessingEvent(CHANGE)).resolves.toBeUndefined();
    await expect(
      events.recordErrorEvent(CHANGE, {}, "boom")
    ).resolves.toBeUndefined();
    await expect(events.recordSuccessEvent(CHANGE)).resolves.toBeUndefined();
    await expect(events.recordCompleteEvent(CHANGE)).resolves.toBeUndefined();
    await expect(
      events.recordPendingEvent(CHANGE, {})
    ).resolves.toBeUndefined();
    await expect(events.recordRetryEvent(CHANGE, {})).resolves.toBeUndefined();
