...
import { queryRange, type Slice, type WorkerStats } from "./common";
...
export class ShardedCounterController {
  private workersRef: CollectionReference;
  private db: Firestore;
  constructor(
    private controllerDocRef: DocumentReference,
  ...
  public async aggregateContinuously(
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const unsubscribeControllerListener = this.controllerDocRef.onSnapshot(
        (snap) => {
          if (snap.exists) {
      ...
      const unsubscribeSliceListener = queryRange(
        this.db,
    ...
    try {
      const status = await this.db.runTransaction(async (t) => {
        ...
        const promises = plans.map(async (plan) => {
          ...
          let counter: DocumentSnapshot = null;
          try {
  ...
  public async rescheduleWorkers() {
    ...
    await this.db.runTransaction(async (t) => {
      ...
      let query: QuerySnapshot = null;
      try {
        query = await t.get(this.workersRef.orderBy(FieldPath.documentId()));
      } catch (err) {
        logger.log("Failed to read worker docs.", err);
        await events.recordErrorEvent(err as Error);
        throw Error("Failed to read worker docs.");
      }
      const shardingInfo: WorkerShardingInfo[] = await Promise.all(
