...
export class ShardedCounterWorker {
  ...
  // worker state
  private shards: QueryDocumentSnapshot[] = null;
  ...
  constructor(
    private metadoc: DocumentSnapshot,
  ...
  public run(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const writeStats = async () => {
        this.allPaths.sort();
        const splits = this.allPaths.filter(
    ...
    const promises = plans.map(async (plan) => {
      try {
        const paths = await this.db.runTransaction(async (t) => {
          ...
          let counter: DocumentSnapshot;
          let metadoc: DocumentSnapshot;
