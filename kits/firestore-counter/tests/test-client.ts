...
/**
 * Mirrors the sharded counter client shipped with the extension
 * (`clients/node/index.js`), so the kit is exercised through the same write
...
export class Counter {
   ...
   */
  constructor(doc: any, field: string) {
    this.shards = {};
   ...
   */
  onSnapshot(
    observable: (snap: { exists: boolean; data: () => number }) => void
  ) {
  ...
  incrementBy(val: number) {
    const increment = FieldValue.increment(val);
