...
describe("syncQueuePath", () => {
  ...
  test("returns the bare name when neither region variable is set, leaving the location to the SDK default", () => {
    expect(syncQueuePath()).toBe("syncBigQuery");
  });

  test("treats empty region variables as unset", () => {
...
describe("enqueueSyncTask", () => {
  ...
  test("targets the bare function name; the admin SDK adds the kit prefix", async () => {
    ...
    expect(enqueue).toHaveBeenCalledWith(change, { id: sha256("evt-1") });
  });

  test("hashes the event id into the task id so a retry cannot double-buffer", async () => {
    const enqueue = vi.fn().mockResolvedValue(undefined);
    mockQueue(enqueue);
  ...
  test("event ids that differ only in punctuation get distinct task ids", async () => {
    const enqueue = vi.fn().mockResolvedValue(undefined);
    mockQueue(enqueue);

    await enqueueSyncTask(makeChange({ eventId: "a.b" }), 3);
    await enqueueSyncTask(makeChange({ eventId: "a/b" }), 3);

    const ids = enqueue.mock.calls.map((call) => call[1].id);
