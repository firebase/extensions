...
traffic reached the Pub/Sub function. `upsertTransferConfig` keeps `ALLOW_ALL`,
as the extension's task queue did: Cloud Tasks dispatches to the function's
public URL. Existing kit deployments adopt the restrictions on their next deploy.
